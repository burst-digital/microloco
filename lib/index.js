...
function createInterpolationPostprocessor(interpolationValues) {
...
function t(lookupKey, activeTranslations, processedGlobalOptions, processedTranslationOptions) {
    ...
    if (typeof translation === 'string') {
        return postprocessTranslation(translation, [
            createInterpolationPostprocessor(processedTranslationOptions.interpolationValues)
        ].concat(Array.isArray(processedTranslationOptions.postprocessors) ? processedTranslationOptions.postprocessors.slice() : []));
    }
...
function setupT(translations, globalOptions) {
    var processedGlobalOptions = __assign({}, constants_1.DEFAULT_INIT_OPTIONS, globalOptions);
    var activeTranslations = getActiveTranslations(translations, processedGlobalOptions.lang, processedGlobalOptions.defaultLang);
    return function (lookupKey, translationOptions) {
        ...
        return t(lookupKey, activeTranslations, processedGlobalOptions, processedTranslationOptions);
    };
}
